...
contract MscpVesting is Ownable {
    ...
    function getTotalReleased() external view returns(uint) {
        if(balances[msg.sender].strategicInvestor){
            if(balances[msg.sender].claimedBonus)
                return SUPPLY_STRATEGIC - balances[msg.sender].remainingCoins + 2000000 * 10**18;
            return SUPPLY_STRATEGIC - balances[msg.sender].remainingCoins;
        } else {
            if(balances[msg.sender].claimedBonus)
                return SUPPLY_PRIVATE - balances[msg.sender].remainingCoins + 1500000 * 10**18;
            return SUPPLY_PRIVATE - balances[msg.sender].remainingCoins;
        }
    }
